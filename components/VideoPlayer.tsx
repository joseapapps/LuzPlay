      ...
      `}>
        {adStep === 'pre' && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white bg-gradient-to-br from-indigo-900 to-black">
            <div className="relative w-full h-full">
              <img 
                src="https://picsum.photos/seed/pre-roll/1280/720" 
                alt="Ad" 
                className="w-full h-full object-cover opacity-50"
              />
              <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
                <h2 className="text-3xl font-black mb-2 italic">PATROCÍNIO EXCLUSIVO</h2>
                <p className="text-xl mb-8 opacity-80">Conheça o novo LuzPlay Premium.</p>
                <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/20">
                  <p className="text-sm font-medium mb-4">O seu vídeo começará em breve...</p>
                  <button 
                    disabled={countdown > 0}
                    onClick={skipAd}
                    className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold transition-all
                      ${countdown > 0 ? 'bg-slate-700 text-slate-400 cursor-not-allowed' : 'bg-brand-primary text-white hover:scale-105'}
                    `}
                  >
                    {countdown > 0 ? `Pular em ${countdown}s` : <><SkipForward /> Pular Anúncio</>}
                  </button>
                </div>
              </div>
            </div>
        ...
        {adStep === 'video' && videoId && (
          <iframe
            ...
            allowFullScreen
            onLoad={() => {
              // Simulação de término de vídeo após algum tempo (em app real usaria YouTube IFrame Player API)
              setTimeout(() => {
                // setAdStep('post');
              }, 300000); // 5 min
            }}
        ...
        {adStep === 'post' && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white bg-slate-900">
            ...
            <div className="mt-8">
              <img src="https://picsum.photos/seed/post/300/100" className="rounded shadow-lg" alt="Offer" />
            </div>
          </div>
        )}
      </div>

      {/* Info Panel Below Player (Desktop) */}
      {!isVertical && adStep === 'video' && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-full max-w-6xl px-4 hidden lg:block">
          <div className="bg-black/40 backdrop-blur-md p-6 rounded-2xl border border-white/10">
            <h1 className="text-2xl font-bold text-white">{video.title}</h1>
            <p className="text-white/60 text-sm mt-2">{video.description}</p>
          </div>
        </div>
      )}
    </div>
